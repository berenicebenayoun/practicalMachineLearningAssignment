...
There are 160 variables and 19622 measurements. However, a number of variables seem to have more than 19000 NA values, either depicted as "NA", the empty string "" or "#DIV/0!" which need to be excluded, as then can't be too informative in the model building.
```{r, cache=TRUE}
...
Let's now exlude the NA-rich variables, as well as the first five columns, which should not be used for the model because they would be irrelevant to our question: these are linked to time of measurement (**timestamp** values), name of exerciser and the number of the measurement.
```{r, cache=TRUE}
...
Now, let's look at the correlation of the remaining variables to estimate how much information is provided for model construction.
```{r, cache=TRUE, fig.width=8, fig.height = 2,}
library('pheatmap')
...
```{r,echo = FALSE}
my.final.acc <- my.confus.mat$overall[1]
```
